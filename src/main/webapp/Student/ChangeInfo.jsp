...
<!-- 接收值信息
		...
		学院专业联动
	输出注册信息
		学生姓名、年龄、性别、学院编号、学生学历、邮箱、号码、地址等
...
<script type="text/javascript">	
	...
	var classMajorId = new Array();
	// var school = new Array();
	function CancelRegister(){
		window.location.href="StudentLogin.jsp";
	}
	function getCollegeSelected(){
				//获取被选学院的value值
				var college = document.getElementById("college");
				var collegeIndex = college.selectedIndex;
				var majorNameList ="专业名称：<select id=\"major\" name=\"major\" onchange=\"getMajorSelected();\">";
				for (var i = 0; i <${majorCount}; i++){
					if(String(college.options[collegeIndex].value) == String(collegeId[i])){
						majorNameList +="<option value=\""+majorId[i]+"\">"+majorName[i]+"</option>";
					}
				}
				majorNameList +="</select>";
				document.getElementById("majorNameList").innerHTML = majorNameList;
			}
	function getMajorSelected(){
	    var majorId = document.getElementById("major");
		var majorIndex = majorId.selectedIndex;
		var classIdList = "班级编号：<select id=\"classes\" name=\"classes\" onchange=\"getClassesSelected();\">";
		for (var i = 0; i<${classCount}; i++ ){
            if(String(majorId.options[majorIndex].value) == String(classMajorId[i])){
                classIdList += "<option value=\""+classId[i]+"\">"+classId[i]+"</option>";
            }
        }
        classIdList += "</select>";
        document.getElementById("classIdList").innerHTML = classIdList;
	}
	function getClassesSelected(){
...
<div class="layui-layout layui-layout-admin">
    <div class="layui-header">
        ...
        <ul class="layui-nav layui-layout-left">
            <li class="layui-nav-item"><a href="/Student/StudentMain?id=3" title="刷新"><i class="layui-icon layui-icon-refresh-3"></i></a></li>
    ...
    <c:forEach items="${majorList}" var="major">
        <script type="text/javascript">
            majorName.push("${major.majorName}");
            majorId.push("${major.majorId}");
            collegeId.push("${major.collegeId}");
            collegeName.push("${major.collegeName}");
        </script>
    </c:forEach>
    <c:forEach items = "${classList}" var="classes">
        <script>
            classId.push("${classes.classId}");
            classMajorId.push("${classes.major.majorId}");
        </script>
    </c:forEach>
    <div class="layui-body">
        <div class="box">
            <form action="StudentChangeInfo" method="post">
            <div class="container" align="center">

                <table style="width: 500px;" class="table table-striped table-hover table-bordered table1">
                    <%--<h1>修改信息</h1>--%>
                    <tbody>
                    <tr><td class="p1">姓名</td><td class="p2"><input type="text" name="name" value="${Student.studentName}"></td></tr>
                    <tr><td class="p1">年龄</td><td class="p2"><input type="text" name="age" value="${Student.studentAge}"></td></tr>
                    ...
                    <tr><td class="p1">邮箱</td><td class="p2"><input type="text" name="mail" value="${Student.studentEmail}"}></td></tr>
                    <tr><td class="p1">电话</td><td class="p2"><input type="text" name="tel" value="${Student.studentTel}"></td></tr>
                    </tbody>
                </table>
                <table style="width: 600px;" class="table table-striped table-hover table-bordered">
                    <%--<h1>修改信息</h1>--%>
                    <tbody>
                    <tr><td class="p1">学历</td><td class="p2"><select id="xl" name="xl">
                        <option value="专科">专科</option>
                        <option value="本科" selected="selected">本科</option>
                        <option value="硕士">硕士</option>
                        <option value="博士">博士</option>
                    </select></td></tr>
                    <tr><td class="p1">学院</td><td class="p2"><select id="college" name="college" onchange="getCollegeSelected();" value="${College.collegeName}">
                        <c:forEach items="${collegeList}" var="college">
                            <option value="${college.collegeId}">${college.collegeName}</option>
                        </c:forEach>
                    </select></td></tr>
                    <tr><td class="p1">专业</td><td id="majorNameList" class="p2"><select id="major" name="major"  onchange="getMajorSelected();" onselect="">
                        <c:forEach items="${majorList}" var="major">
                            <option value="${major.majorId}">${major.majorName}</option>
                        </c:forEach>
                    </select></td></tr>
                    <tr><td class="p1">班级</td><td id="classIdList" class="p2"><select id="classes" name="classes" onchange="getClassesSelected();" value="${Classes.classId}">
                        <c:forEach items="${classList}" var ="classes">
                            <option value="${classes.classId}">${classes.classId}</option>
                        </c:forEach>
                    </select></td></tr>
                    <tr><td colspan="2" align="center"><input class="submit" type="submit" value="修改"></td></tr>
...
<script>
	...
	<script>
		//省级别
		var proviceName ="省：<select id=\"proviceId\" name=\"provice\" onchange=\"getProviceIndex();\">";
		for (var i = 0; i < provice.length; i++) {
			proviceName += "<option>"+provice[i]['name']+"</option>";
		}
		proviceName+="</select>"
		document.getElementById("provice").innerHTML= proviceName; 
		//provice[1]['city'][0]['districtAndCounty'][0];

		function  getProviceIndex(){
			var proviceId = document.getElementById("proviceId");
			var proviceIndex = proviceId.selectedIndex;//获取被选省份的索引index
			var cityName ="市：<select id=\"cityId\" name=\"city\" onchange=\"getCounty("+proviceIndex+");\">";
			for (var i = 0; i < provice[proviceIndex]['city'].length; i++) {
				cityName+="<option>"+provice[proviceIndex]['city'][i]['name']+"</option>";
			}
			cityName+="</select>";
			document.getElementById("city").innerHTML= cityName; 
		}

		function getCounty(proviceIndex){
			var cityId = document.getElementById("cityId");
			var cityIndex = cityId.selectedIndex;
			var countyName="区/县:<select id=\"countyId\" name=\"county\">";
			for (var i = 0; i < provice[proviceIndex]['city'][cityIndex]['districtAndCounty'].length; i++) {
				countyName+="<option value=\""+provice[proviceIndex]['city'][cityIndex]['districtAndCounty'][i]+"\">"+provice[proviceIndex]['city'][cityIndex]['districtAndCounty'][i]+"</option>"
			}
			countyName+="</select>";
			document.getElementById("county").innerHTML= countyName;
		}
	</script>
